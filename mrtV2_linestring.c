...
int mrtV2(char i_filename[], char config_fn[]) {
  ...
  */
  ...
  for (h=0; h<num_contour_levels; h++) {
    ...
    linestrings = (struct index_pair**)malloc(sizeof(struct index_pair*));
    waypoints = (int*)malloc(sizeof(int));
	...
	if (!contained(numpoints+1, current_ip, contour_ips)) {
	  ...
	  if(multipath == 0) {
	    ...
	    if ((indexe[h][i][j].xind != -1) && (rfr != 0)) {
	      ...
	      //unhandled case... break it off
	      linestrings[numlinestrings] = (struct index_pair*)malloc((numpoints+1)*sizeof(struct index_pair));
	      waypoints[numlinestrings] = numpoints+1;
	      for (k=0; k<numpoints+1; ++k) {
		//print out the linestring
		printf("linestring %d pair %d i %d j %d\n", numlinestrings, k, contour_ips[k].xind, contour_ips[k].yind);
		linestrings[numlinestrings][k] = contour_ips[k];
	...
	if (contained(numpoints+1, current_ip, contour_ips)) {
	  ...
	  //dump the points to the used up array
	  for (k=0; k<numpoints+1; ++k) {
	    used_up_ips[num_used_up_ips] = contour_ips[k];
	    num_used_up_ips++;
	    used_up_ips = (struct index_pair*)realloc(used_up_ips, (num_used_up_ips+1)*sizeof(struct index_pair));
	  ...
	  int a;
	  printf("test\n");
	  if ((numpoints > min_contour_points) && ((abs(contour_ips[0].yind - contour_ips[numpoints].yind) < 2) && (abs(contour_ips[0].xind - contour_ips[numpoints].xind) < 2))) {
	    for (k=0; k<numpoints+1; ++k) {
	    ...
	    //if not enough points in the contour, call it a linestring
	    linestrings[numlinestrings] = (struct index_pair*)malloc((numpoints+1)*sizeof(struct index_pair));
	    waypoints[numlinestrings] = numpoints+1;
	    for (k=0; k<numpoints+1; ++k) {
	      //print out the linestring
    ...
    //ok at this point we may have a set of linestrings that could be assembled into polygons
	...
	printf("b: %d  x: %d  y: %d\n", b, linestrings[a][b].xind, linestrings[a][b].yind);
      ...
      //eventually the accumulated linestrings should become polygons as they are added together
      if ((abs(linestrings[a][0].xind - linestrings[a][waypoints[a]-1].xind) < 2) && (abs(linestrings[a][0].yind - linestrings[a][waypoints[a]-1].yind) < 2)) {
	if (waypoints[a] >= min_contour_points) {
