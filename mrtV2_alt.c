...
int mrtV2(char i_filename[], char config_fn[]) {
  ...
  */
  ...
  for (h=0; h<num_contour_levels; h++) {
    ...
    int num_multipaths=0; //number of multipaths we've come across per contour
    ...
    struct index_pair *multipaths; //an array of multipaths that we come across
    
    multipaths = (struct index_pair*)malloc(sizeof(struct index_pair));
    ...
    //in this approach to the code, we want to start by identifying multipaths
    //multipaths are where there are more than one route in or out
    for (i=0; i<num_lats;i++) {
      for (j=0; j<num_lons;j++) {
	...
	if (indexes1[h][i][j].xind != -1)
	  ++kntr;
	if (kntr > 2) {
	  multipaths[num_multipaths] = current_ip;
	  num_multipaths++;
	  multipaths = (struct index_pair*)realloc(multipaths, (num_multipaths+1)*sizeof(struct index_pair));
	}
      }
    }

    //now that the number of multipaths has been determined we'll keep track of each of their possible routes
    //unlike regular paths with one route in and one route out, mulltipaths will be in multiple polygons

    //to avoid declaring a length zero multipath_routes array, we'll set a flag and ensure at least some length is set
    int no_multipaths = 0;
    if (num_multipaths == 0) {
      no_multipaths = 1;
      num_multipaths = 1;
    }
    int multipath_routes[num_multipaths][4];

    for (i=0; i < num_multipaths; i++) {
      if (!no_multipaths)
	printf("multipath %d x: %d y: %d\n", i, multipaths[i].xind, multipaths[i].yind);
      for (j=0; j < 4; j++) {
	multipath_routes[i][j] = 1;
      }
    }
	...
	//let's see if we have a point that referred us here...
	...
	if(reference.xind == -1) {
	  ...
	  //make an coordinate array to hold the eventual contour coordinates
